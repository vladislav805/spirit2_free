...
// Patchram data !!!!
// unsigned char make_over_60k_for_hal_file_size_get [32768] = {0};
#include "hcd/hcd_bch.c"

const char * copyright = "Copyright (c) 2011-2015 Michael A. Reid. All rights reserved.";

// Default 0 = UART, 1 = Bluedroid SHIM
int shim_hci_enable = 1;
...
// 0 = stopped
// 1 = started
// 2 = not needed but thread still alive
static volatile uint8_t thread_state = 0;
...
static void* bt_server_thread(void* arg) {
	...
	// While needed and running...
	while (thread_state == 1) {
		logd("bt_server_thread thread_state = needed and running");

		// !! Doesn't return if no packets sent
		gen_server_loop(hci_port, poll_tmo_ms);

		if (thread_state == 1) {
			// Keep trying if error return
			sleep(1);
		}
	}
...
static void hcib_preload_result_cb(TRANSAC transac, bt_hc_preload_result_t result) {
...
static void hcib_postload_result_cb(TRANSAC transac, bt_hc_postload_result_t result) {
...
// Low Power Mode enable/disable callback
static void hcib_lpm_result_cb(bt_hc_lpm_request_result_t result) {
...
static void hcib_hostwake_ind_cb(bt_hc_low_power_event_t event) {
...
static char * hcib_alloc_mem_cb(int size) {
...
static void hcib_dealloc_mem_cb(TRANSAC transac) {
...
static int hcib_data_ind_cb(TRANSAC transac, char* p_buf, int len) {
	if (
		// Broadcom 3F 15 for FM
		(len >= 5 && p_buf[3] == 0x15 && p_buf[4] == 0xfc) ||

		// Broadcom 3F 00 for LG G2 audio
		(len >= 5 && p_buf[3] == 0x00 && p_buf[4] == 0xfc)
	) {
		num_hci_cback++;
...
static int hcib_tx_result_cb(TRANSAC transac, char* p_buf, bt_hc_transmit_result_t result) {
...
static void server_thread_stop() {
	// If thread_state == 1 = Running and active
	if (thread_state == 1) {
		// thread_state =     2 = Running and not needed
		thread_state = 2;
	}
}
...
static int server_thread_start() {
	...
	thread_state = 1; // = Running and active
...
char* gv_mem_alloc() {
	if (hcib_alloc_mem_cb != NULL) {
		// Allocate: BT_HC_HDR_SIZE + HCI_CMD_PREAMBLE_SIZE
		return (hcib_alloc_mem_cb (MAX_HCI + 24));
	}
}
